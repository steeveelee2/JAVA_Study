...
public class MemberManager {
	...
	{ // 초기화 블럭
		...
		mList.add(new Member("홍길동", "hong1234"
				, "pass01", "01011112222"
				, "test1@test.com", 'M', 10));
		mList.add(new Member("신사임당", "sinsa001"
				, "pass02", "01022223333"
				, "test2@test.com", 'F', 20));
		mList.add(new Member("김유신", "kim002"
				, "pass03", "01011223344"
				, "test3@test.com", 'M', 30));
		mList.add(new Member("임꺽정", "Limgguck003"
				, "pass04", "01044442222"
				, "test4@test.com", 'M', 40));
	...
	public void printAllMember(){
		...
		for(Object m : mList) {
			System.out.println(((Member)m).memberInfo());
		}
	...
	public int selectMember(){
		...
		for(int i = 0; i < mList.size(); i++) {
			if(((Member)mList.get(i)).getId().equals(id)) {
				idx = i;
				break;
			}
		}
	...
	public void updateMemberPassword(){
		...
		if(idx < 0 ){
			...
			System.out.println(((Member)mList.get(idx)).getPassword());
			// System.out.println(mArr[idx].getPassword());
			System.out.print("변경할 비밀번호 : ");
			// mArr[idx].setPassword(sc.next());
			((Member)mList.get(idx)).setPassword(sc.next());
		}
	}
	...
	public void updateMemberEmail(){
		...
		if(idx < 0){
			...
			/*System.out.println("현재 이메일 : "
					+ mArr[idx].getEmail());*/
			System.out.println("현재 이메일 : "
			+ ((Member)mList.get(idx)).getEmail());
			System.out.println("변경할 이메일 입력 : ");
			// mArr[idx].setEmail(sc.next());
			((Member)mList.get(idx)).setEmail(sc.next());
		}
	}
	...
	public void printOneMember(){
		...
		if(idx < 0){
			...
			System.out.println(((Member)mList.get(idx)).memberInfo());
